<template>
  ...
  <div id="searchEngineD" class="listStyle">
    ...
    <div class="top">
      ...
      <!-- 法规层级 -->
      <regulations-hierarchy v-if="flag" ref="regulations" @getSearchLists="getSearchLists"></regulations-hierarchy>
      <!-- 搜索框 -->
        ...
        <div class="selectNavS">
          ...
          <van-collapse v-model="activeNames" accordion>
            ...
            <van-collapse-item title="行业检索" name="2">
              ...
              <div class="btn" @click="searchDatas(2)"><van-button block icon="search" style="border-radius: 30px;" type="info">查询</van-button></div>
            </van-collapse-item>
...
export default {
  data() {
    return {
      ...
      SearchTimes: 1,
      isFilter: false,
      clickFlag: true, // 点击的是哪个的tagpc确定按钮
      clickFlagC: true,
      clickFlagI: true,
      fixedTagid: '', // 固定tagid
      fixedTagidC: '', // 固定tagid
      fixedTagidI: '', // 固定tagid
      clickConfirmBtn: true,  // 带上tagpc
      clickConfirmBtnC: true,  // 带上tagpc
      clickConfirmBtnI: true,  // 带上tagpc
      showAllTagpc: [], // 保存tagpc
      showAllTagpcC: [],
      showAllTagpcI: [],
    }
  },
  ...
  methods: {
    ...
    getSearchLists(tagpc) {
      ...
      // console.log('传递给父组件的数据tagpc==》', tagpc, this.showAllTagpc)
      this.isFilter = true
      this.tagpc = tagpc
      // if (tagpc.length > 0) {
      //   this.tagpc = tagpc
    ...
    getData(tagid, stime, etime, searchText) {
      ...
      this.$http.reqGetGeneralFiles(args, nodes).then(response => {
        ...
        this.t2 = this.t2.sort((a, b) => a.id - b.id)
        this.t3 = this.t3.sort((a, b) => a.id - b.id)
        ...
        // console.log('综合检索66666proAndCityData=>', this.proAndCityData)
    ...
    getData2Common(tagid, stime, etime, searchText) {
      ...
      this.$http.reqGetIndustryCommonFiles(args, nodes).then(response => {
        ...
        this.t1C =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t1.length > 0 ? response.tbls[0].divisiontag.t1 : []
        this.t2C =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t2.length > 0 ? response.tbls[0].divisiontag.t2 : []
        this.t3C =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t3.length > 0 ? response.tbls[0].divisiontag.t3 : []

        this.t2C = this.t2C.sort((a, b) => a.id - b.id)
        this.t3C = this.t3C.sort((a, b) => a.id - b.id)

        this.proAndCityDataC = {
          t1: this.t1C.length > 0 ? this.t1C : [],
          t2: this.t2C.length > 0 ? this.t2C : [],
          t3: this.t3C.length > 0 ? this.t3C : []
        }
        // console.log('行业检索 通用proAndCityData=>', this.proAndCityDataC)
        // this.$refs.regulations.processData(this.proAndCityDataC)
        if (this.clickConfirmBtnC) {
          this.tagpc = this.filterData(this.proAndCityDataC)
          this.showAllTagpcC = []
          this.showAllTagpcC = this.tagpc
        }

        this.commonTotal5 = Math.ceil(response.tbls[0].total/10)
        this.commonTotal5Lists = response.tbls[0].total
        this.classifytag = response.tbls[0].classifytag
        this.fixedTagidC = this.getTagid(this.classifytag)
        // console.log('this.fixedTagidC', this.fixedTagidC)
    ...
    getData2Industry(tagid, stime, etime, searchText) {
      ...
      // console.log('行业检索 专属的数据====》', args)
      let nodes = this.$baseConfig.code
      this.updateIndustryDatas = args

      this.$http.reqGetIndustryFiles(args, nodes).then(response => {
        console.log('行业检索 专属response', response)
        this.showLoding = false
        this.industryLists = response.tbls[0].items !== null ? response.tbls[0].items : []
        this.t1I =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t1.length > 0 ? response.tbls[0].divisiontag.t1 : []
        this.t2I =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t2.length > 0 ? response.tbls[0].divisiontag.t2 : []
        this.t3I =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t3.length > 0 ? response.tbls[0].divisiontag.t3 : []
        this.t2I = this.t2I.sort((a, b) => a.id - b.id)
        this.t3I = this.t3I.sort((a, b) => a.id - b.id)
        this.proAndCityDataI = {
          t1: this.t1I.length > 0 ? this.t1I : [],
          t2: this.t2I.length > 0 ? this.t2I : [],
          t3: this.t3I.length > 0 ? this.t3I : []
        }
        // console.log('行业检索 专属proAndCityData=>', this.proAndCityDataI)
        // this.$refs.regulations.processData(this.proAndCityDataI)
        if (this.clickConfirmBtnI) {
          this.tagpc = this.filterData(this.proAndCityDataI)
          this.showAllTagpcI = []
          this.showAllTagpcI = this.tagpc
        }

        this.industryTotal5 = Math.ceil(response.tbls[0].total/10)
        this.industryTotal5lists = response.tbls[0].total
        this.classifytag = response.tbls[0].classifytag
        this.fixedTagidI = this.getTagid(this.classifytag)
        // console.log('this.fixedTagidI', this.fixedTagidI)
    ...
    getDataRadio1(tagid, stime, etime, searchText) {
      ...
      this.$http.reqGetTaxBreakFiles_item(args, nodes).then(response => {
        ...
        // console.log('税收优惠项目proAndCityData=>', this.proAndCityData)
        // this.$refs.regulations.processData(proAndCityData)
        if (this.clickConfirmBtn) {
          this.tagpc = this.filterData(this.proAndCityData)
          this.showAllTagpc = []
          this.showAllTagpc = this.tagpc
        ...
        this.fixedTagid = this.getTagid(this.classifytag)
        // console.log('this.fixedTagid', this.fixedTagid)
    ...
    getDataRadio2(tagid, stime, etime, searchText) {
      ...
      this.$http.reqGetTaxBreakFiles_taxes(args, nodes).then(response => {
        ...
        // console.log('税收优惠税种proAndCityData=>', this.proAndCityData)
        // this.$refs.regulations.processData(proAndCityData)
        if (this.clickConfirmBtn) {
          this.tagpc = this.filterData(this.proAndCityData)
          this.showAllTagpc = []
    ...
    getData5(tagid, stime, etime, searchText) {
      ...
      this.$http.reqGetInOutFiles(args, nodes).then(response => {
        this.flag = true
        console.log('进出口返回的数据response', response)
        this.showLoding = false
        this.lists4 = response.tbls[0].items !== null ? response.tbls[0].items : []
        this.t1 =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t1.length > 0 ? response.tbls[0].divisiontag.t1 : []
        this.t2 =  response.tbls[0].divisiontag.length !== 0 && response.tbls[0].divisiontag.t2.length > 0 ? response.tbls[0].divisiontag.t2 : []
        ...
        // console.log('进出口proAndCityData=>', this.proAndCityData)
    ...
    // 获取选中的tagid 点击法规层级时能够保持选中状态
    getTagid(data) {
      // console.log('固定获取选中的tagiddata=====>', data)
      let fixedTagid = ''
      data.forEach(item => {
        if (item.children && item.children.length > 1) {
          fixedTagid = item.id
        } else if (item.children && item.children.length === 1 && item.children[0].children) {
          item.children.forEach(item1 => {
            if (item1.children && item1.children.length > 1) {
              fixedTagid = item1.id
            } else if (item1.children && item1.children.length === 1 && item1.children[0].children) {
              item1.children.forEach(item2 => {
                if (item2.children && item2.children.length > 1) {
                  fixedTagid = item2.id
                } else if (item2.children && item2.children.length === 1 && item2.children[0].children) {
                  item2.children.forEach(item3 => {
                    if (item3.children && item3.children.length > 1) {
                      fixedTagid = item3.id
                    } else if (item3.children && item3.children.length === 1 && item3.children[0].children) {
                      item3.children.forEach(item4 => {
                        // console.log('66666666666666666', item4)
                      })
                    } else {
                      fixedTagid = item3.children[0].id
                    }
                  })
                } else {
                  fixedTagid = item2.children[0].id
                }
              })
            } else {
              fixedTagid = item1.children[0].id
            }
          })
        } else {
          fixedTagid = item.children[0].id
        }
      })
      return fixedTagid
    },
    ...
    searchDatas(num) {
      ...
      this.clickConfirmBtnC = true
      this.clickConfirmBtnI = true
      this.$refs.classification.clearSearchValue()
      this.clickFlag = true
      this.clickFlagC = true
      this.clickFlagI = true
      this.flag = true
      this.SearchTimes = 1
      this.isFilter = false
      this.queryBtn = num
    ...
    alertCengji() {
      let active = this.$store.state.commonSearchData.active
      if (this.flag && (this.queryBtn === 1 || this.queryBtn === 5)) {
        this.$refs.regulations.processData(this.proAndCityData)
      } else if (this.flag && this.queryBtn === 3 && (this.radio === '1' || this.radio === '2')) {
        this.$refs.regulations.processData(this.proAndCityData)
      } else if (this.flag && this.doubleLists && active === 0) {
        this.$refs.regulations.processData1(this.proAndCityDataC)
      } else if (this.flag && this.doubleLists && active === 1) {
        this.$refs.regulations.processData1(this.proAndCityDataI)
      } else {
        this.$notify({ type: 'warning', message: '请先搜索文件后使用' })
      }
    },
    // 点击一键上传
    updateOrDelete() {
      console.log('点击了地方版的一键上传this.tagpc6666==>', this.tagpc)
      // console.log('点击了地方版一键上传6868999', this.updateDatas)
      // console.log('this.updateDatas', this.updateDatas.fliter.tagpc)

      // this.updateDatas.fliter.tagpc = this.tagpc
      // http://192.168.8.31:20201
      ...
      if (this.userId) {
        if (this.queryBtn === 0) {
          ...
          this.updateCommonDatas.fliter.tagpc = this.tagpc
          this.$refs.uploadFile.updateOrDelete(this.updateCommonDatas, '行业检索通用规定')
        } else if (this.queryBtn === 2 && this.industryFlag) {
          this.updateIndustryDatas.fliter.tagpc = this.tagpc
      ...
      //     this.$notify({ color: '#fff', background: '#b99573', message: '请先搜索文件后上传' })
      //   } else if (this.queryBtn === 1) {
      //     console.log('点击了地方版一键上传this.updateDatas', this.updateDatas)
      //     this.$refs.uploadFile.updateOrDelete(this.updateDatas, '综合检索')
      //   } else if (this.queryBtn === 2 && this.commonFlag) {
      //     this.$refs.uploadFile.updateOrDelete(this.updateCommonDatas, '行业检索通用规定')
      //   } else if (this.queryBtn === 2 && this.industryFlag) {
      ...
      //     this.$refs.uploadFile.updateOrDelete(this.updateIndustryDatas, '税收优惠行业专属')
      //   } else if (this.queryBtn === 4 && this.commonFlag) {
      //     console.log('222this.updateCommonDatas=>', this.updateCommonDatas)
      //     this.$refs.uploadFile.updateOrDelete(this.updateCommonDatas, '条约检索通用规定')
      //   } else if (this.queryBtn === 4 && this.industryFlag) {
      //     this.$refs.uploadFile.updateOrDelete(this.updateIndustryDatas, '条约检索专属规定')
      //   } else if (this.queryBtn === 5) {
    ...
    changeTagid(val) {
      ...
      let active = this.$store.state.commonSearchData.active
      // console.log('active选中的tabs', active)
      if (active === 0) {
        this.clickFlagC = false
      } else if (active === 1) {
        this.clickFlagI = false
      }
      this.tagid = val
      this.backStatus()
      this.$refs.listCont.toggle([])
    ...
    changeValue4(value) { // 进出口税收检索的类别切换
      this.value4 = value
    },
    // 综合检索的文件效力改变
    changeSearch1 (value) {
